package com.csrg.syr.parentControl.child.GUI;
...
public class ChildSet extends Activity {
	...
	private static String TAG = "ChildSet";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * show the map location of childset
		 */
		this.btmap = (Button) findViewById(R.id.buttonShowMap);
		this.btmap.setOnClickListener(new View.OnClickListener() {
			public void onClick(View arg0) {
				Intent l = new Intent(ChildSet.this, MapViewActivity.class);
				startActivity(l);
				if (D)
					Log.d(TAG, "clicked map show");
			}
		});
	}
		...
		alertDialogBuilder
				...
				.setPositiveButton("OK", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
						...
						if (D)
							Log.d("authentication", username.getText()
									.toString());
						if (D)
							Log.d("authentication", password.getText()
									.toString());

						// get the password saved and compare with enterd
						// password
						...
						if (log.username.equals(username.getText().toString())) {
							...
							if (log.password.equals(password.getText()
									.toString())) {
								if (D)
									Log.d("checking", "passowrd matched");
								Intent settingsActivity = new Intent(
										getBaseContext(), DeviceAdmin.class);
